...
- **Where:** `data/ingestion/` (weather, market, satellite) and `data-pipelines/ingestion/` (physical paths under `data-eng/`; root symlinks keep short paths).
...
- Orchestration: Composer/Airflow DAGs in `data-eng/airflow/dags/` (scheduled in GCP; symlink `infra/` → `data-eng/infra` for Terraform).
...
- **SQL lint (`sql-lint.yml`):** Lint SQL in `data/sql`, pipeline SQL, and `data-eng/dbt/models` when those paths change.
- **ML tests (`ml-tests.yml`):** Run tests under `ml/` when ML code or requirements change.
- **Data eng (`data-eng-*.yml`):** Terraform fmt, dbt YAML sanity, DAG `py_compile`, Airbyte helper compile when the matching `data-eng/` subtree changes.
